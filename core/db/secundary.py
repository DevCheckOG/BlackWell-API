...
SECUNDARY_CLIENT : AgnosticClient = AsyncIOMotorClient(os.environ['MongoDB'], maxPoolSize= None)
QUEUE_HISTORY : AgnosticClient = SECUNDARY_CLIENT.get_database('messages').get_collection('queue history')
TEMP_USERS : AgnosticClient = SECUNDARY_CLIENT.get_database('users').get_collection('users temporal')

# Temporary Users Section - Secondary DB

async def find_possible_user(username: str = '', email: str = '') -> bool:

    user : Dict[str, Any] | None = await USERS.find_one({'username' : {'$regex' : username, '$options' : 'i'}, 'email' : {'$regex' : email, '$options' : 'i'}})
    temp_user : Dict[str, Any] | None = await TEMP_USERS.find_one({'username' : {'$regex' : username, '$options' : 'i'}, 'email' : {'$regex' : email, '$options' : 'i'}})
...
async def is_valid_verification_code(code: str = '') -> bool:    
    ...
    result : Dict[str, Any] | None = await TEMP_USERS.find_one({'verification.code' : code})
    
    return True if isinstance(result, dict) and result['verification']['code'] == code else False

async def get_temp_user(code: str = '') -> Dict[str, Any] | bool:

    result : Dict[str, Any] | None = await TEMP_USERS.find_one({'verification.code' : code})
    
    return result if isinstance(result, dict) and result['verification']['code'] == code else False
...
async def add_message_queue_history(to: str, message: Dict[str, Any]) -> bool:
    ...
    fetch_possible_history : List[Dict[str, Any]] | bool = await get_queue_history(token[0])

    if isinstance(fetch_possible_history, list):

        await QUEUE_HISTORY.update_one(
    ...
    await QUEUE_HISTORY.insert_one({
        ...
        '_id' : token,
        'username' : to,
        'created-at' : datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M'),
        'messages' : [message]
...
def cleaner_temporal_accounts() -> None:
    ...
    TEMP_USERS : Collection = CLIENT.get_database('users').get_collection('users temporal')

    while True:

        if TEMP_USERS.count_documents({}) == 0: continue

        temp_users : List[Dict[str, Any]] = list(TEMP_USERS.find())

        for user in temp_users:

            if datetime.datetime.strptime(user['verification']['remaining'], '%Y-%m-%d %H:%M:%S.%f') <= datetime.datetime.now():
...
def cleaner_queue_history() -> None:
    ...
    CLIENT : MongoClient = pymongo.MongoClient(os.environ['MongoDB'], maxPoolSize= None)
    QUEUE_HISTORY : Collection = CLIENT.get_database('messages').get_collection('queue history')

    while True:

        time.sleep(60 * 60 * 25)

        if QUEUE_HISTORY.count_documents({}) == 0: continue
