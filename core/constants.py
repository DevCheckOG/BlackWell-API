...
class Constants(Enum):
    ...
    EMAIL_AND_PASSWORD_IS_NOT_VALID_OR_USER_NOT_EXISTS: str = (
        "email and password is not valid or user not exists"
    )
    INCORRECT_SYNTAX: str = "incorrect syntax"

    REQUIRED_EMAIL_AND_PASSWORD_IN_THE_GATEWAY: str = (
        "required email and password in the gateway"
    )
    REQUIRED_EMAIL_AND_PASSWORD_AND_USERNAME_IN_THE_GATEWAY: str = (
        "required email and password and username in the gateway"
    )
