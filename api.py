...
from core.db.primary import get_token_with_email_and_password, cleaner_history, set_user_profile, get_user, check_if_user_in_contacts, get_user_profile
...
async def startup() -> None:
    ...
    threading.Thread(target= cleaner_temporal_accounts, name= '| Temporal Accounts |').start()
...
async def gateway(websocket : fastapi.WebSocket, email : str | None, password : str | None) -> None | Any:
    ...
    elif not any(email == conexion['email'] and password == conexion['password'] for conexion in GATEWAY_CONEXIONS):
...
async def login(request : fastapi.Request, data : Login) -> fastapi.responses.JSONResponse:
...
async def register(request : fastapi.Request, data : Register) -> fastapi.responses.JSONResponse:

    result : Dict[str, Any] = await UserManager().register_user(data.username, data.email, data.password)
...
@API.post('/verify')
@IPLimiter.limiter(max_calls= 5, time= 30)
async def verify(request : fastapi.Request, data : Verify) -> fastapi.responses.JSONResponse:
...
async def delete_user(request : fastapi.Request, data : DeleteUser) -> fastapi.responses.JSONResponse:
...
@API.post('/user/token')
@IPLimiter.limiter(max_calls= 10, time= 60)
async def token(request : fastapi.Request, data : Token) -> fastapi.responses.JSONResponse:

    result : list[str, Any] | bool = await get_token_with_email_and_password(data.email, data.password)
...
async def set_profile(request : fastapi.Request, data : SetProfile) -> fastapi.responses.JSONResponse:

    if not Parser().is_hex(data.image) or not Parser().check_img_or_video_size(bytes.fromhex(data.image)):
...
async def profile(request : fastapi.Request, data : Profile) -> fastapi.responses.JSONResponse:

    result : Dict[str, Any] | bool = await get_user(data.token)

    if isinstance(result, dict):

        contact_profile : bool = await check_if_user_in_contacts(data.token, data.username)
...
async def send_message(request : fastapi.Request, type : Literal['img', 'video', 'text'] | None, data : SendMessage) -> fastapi.responses.JSONResponse:
    ...
    if type is None:
        ...
        elif not any(data.email == conexion['email'] and data.password == conexion['password'] for conexion in GATEWAY_CONEXIONS):
        ...
        parsed_message : Dict[str, Any] | bool = Parser().parse_video_or_img_message(data.message)

        if not Parser.is_hex(parsed_message['contain']) or not Parser().check_img_or_video_size(bytes.fromhex(parsed_message['contain'])):
...
async def delete_messages(request : fastapi.Request, data : DeleteMessage) -> fastapi.responses.JSONResponse:

    if not any(data.email == conexion['email'] and data.password == conexion['password'] for conexion in GATEWAY_CONEXIONS):
    ...
    result : bool | str = await Gateway.send_message(data.to, {
...
def rate_limit_exceeded(request : fastapi.Request, exc : fastapi.exceptions.HTTPException) -> fastapi.responses.JSONResponse:
...
async def not_found(request : fastapi.Request, exc : fastapi.exceptions.HTTPException) -> fastapi.responses.JSONResponse:
...
async def unprocessable_entity(request : fastapi.Request, exc : fastapi.exceptions.HTTPException) -> fastapi.responses.JSONResponse:
...
async def overloaded(request : fastapi.Request, exc : fastapi.exceptions.HTTPException) -> fastapi.responses.JSONResponse:
...
API.add_exception_handler(fastapi.status.HTTP_422_UNPROCESSABLE_ENTITY, unprocessable_entity)
API.add_exception_handler(fastapi.status.HTTP_404_NOT_FOUND, not_found)
API.add_exception_handler(fastapi.status.HTTP_429_TOO_MANY_REQUESTS, rate_limit_exceeded)
